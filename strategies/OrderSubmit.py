...
class OrderSubmit(bt.Strategy):
    ...
    def next(self):
        ...
        if(self.position.size < 0 and self.data.low[0] < self.lastLow):
            ...
            self.cancel(self.loss)
            self.loss = self.buy(exectype=bt.Order.Stop, size=self.size, price=self.tsl)
        ...
        if(self.position.size > 0 and self.data.high[0] > self.lastHigh):
            ...
            self.cancel(self.loss)
            self.loss = self.sell(exectype=bt.Order.Stop, size=self.size, price=self.tsl)
        ...
        if not self.position and self.order is None:
            ...
            if not isAlligator and not isSupertrend:
                ...
                if  isP or isS1 or isS2 or isR1 or isR2:
                    ...
                    self.loss = self.buy(exectype=bt.Order.Stop, size=self.size, price=self.tsl, parent=self.order)
                    self.win = self.buy(exectype=bt.Order.Stop, size=self.size, price=self.target, parent=self.order)
            ...
            if isAlligator and isSupertrend:
                ...
                if isP or isS1 or isS2 or isR1 or isR2:
                    ...
                    self.loss = self.sell(exectype=bt.Order.StopLimit, size=self.size, price=self.tsl)
                    self.win = self.sell(exectype=bt.Order.Stop, size=self.size, price=self.target)
        if self.order:
        ...
        if self.position:
            if self.datas[0].datetime.time().hour >= 15:
